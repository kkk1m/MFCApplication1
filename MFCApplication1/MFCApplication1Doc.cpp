...
#include "CQuantization.h" // 대화상자 사용을 위한 헤더 선언
...
void CMFCApplication1Doc::OnDownSampling()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j;
	CDownSampleDlg dlg;
...
void CMFCApplication1Doc::OnUpSampling()
{
	int i, j;
...
void CMFCApplication1Doc::OnQuantization()
{
	CQuantization dlg;
	if (dlg.DoModal() == IDOK)
...
void CMFCApplication1Doc::OnSumConstant()
{
...
void CMFCApplication1Doc::OnSubConstant()
{
	// TODO: 여기에 구현 코드 추가.
	CConstantSubDlg dlg;
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			if (m_InputImage[i] - dlg.m_ConstantSub < 0)
				m_OutputImage[i] = 0; // 출력 값이 255보다 크면 255를 출력
			else
				m_OutputImage[i] = (unsigned char)(m_InputImage[i] - dlg.m_ConstantSub);
			// 상수 값과 화소 값과의 뺄셈
		}
	}
...
void CMFCApplication1Doc::OnMulConstant()
{
	CConstantMul dlg;
	int i;
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			if (m_InputImage[i] * dlg.m_ConstantMul > 255)
				m_OutputImage[i] = 255;
			// 곱의 값이 255보다 크면 255를 출력
			else if (m_InputImage[i] * dlg.m_ConstantMul < 0)
				m_OutputImage[i] = 0;
			// 곱의 값이 0보다 작으면 0을 출력
			else
				m_OutputImage[i]
				= (unsigned char)(m_InputImage[i] * dlg.m_ConstantMul);
			// 상수 값과 화소 값 곱셈
		}
	}
...
void CMFCApplication1Doc::OnDivConstant()
{
	// TODO: 여기에 구현 코드 추가.
	CConstantDiv dlg;
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			if (m_InputImage[i] / dlg.m_ConstantDiv > 255)
				m_OutputImage[i] = 255;
			// 나눗셈의 값이 255보다 크면 255를 출력
			else if (m_InputImage[i] / dlg.m_ConstantDiv < 0)
				m_OutputImage[i] = 0;
			// 나눗셈의 값이 0보다 작으면 0을 출력
			else
				m_OutputImage[i]
				= (unsigned char)(m_InputImage[i] / dlg.m_ConstantDiv);
			// 상수 값과 화소 값 나눗셈
		}
	}
}
...
void CMFCApplication1Doc::OnAndOperate()
{
	// TODO: 여기에 구현 코드 추가.
	CConstantAnd dlg;
	int i;
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			// 비트 단위 AND 연산
			if ((m_InputImage[i] & (unsigned char)dlg.m_ConstantAnd) >= 255)
			{
				m_OutputImage[i] = 255;
			}
			else if ((m_InputImage[i] & (unsigned char)dlg.m_ConstantAnd) < 0)
			{
				m_OutputImage[i] = 0;
			}
			else {
				m_OutputImage[i] = (m_InputImage[i]
					& (unsigned char)dlg.m_ConstantAnd);
			}
		}
	}
...
void CMFCApplication1Doc::OnOrOperate()
{
	// TODO: 여기에 구현 코드 추가.
	CConstantOr dlg;
	int i;
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			// 비트 단위 OR 연산
			if ((m_InputImage[i] | (unsigned char)dlg.m_ConstantOr) >= 255) {
				m_OutputImage[i] = 255;
			}
			else if ((m_InputImage[i] | (unsigned char)dlg.m_ConstantOr) < 0) {
				m_OutputImage[i] = 0;
			}
			else {
				m_OutputImage[i] = (m_InputImage[i] |
					(unsigned char)dlg.m_ConstantOr);
			}
		}
	}
...
void CMFCApplication1Doc::OnXorOperate()
{
	// TODO: 여기에 구현 코드 추가.
	CConstantXor dlg;
	int i;
	...
	if (dlg.DoModal() == IDOK) {
		for (i = 0; i < m_size; i++) {
			// 비트 단위 XOR 연산
			if ((m_InputImage[i] ^ (unsigned char)dlg.m_ConstantXor) >= 255) {
				m_OutputImage[i] = 255;
			}
			else if ((m_InputImage[i] ^ (unsigned char)dlg.m_ConstantXor) < 0) {
				m_OutputImage[i] = 0;
			}
			else {
				m_OutputImage[i] = (m_InputImage[i]
					^ (unsigned char)dlg.m_ConstantXor);
			}
		}
	}
...
void CMFCApplication1Doc::OnNegaTransform()
{
	// TODO: 여기에 구현 코드 추가.
	int i;
...
void CMFCApplication1Doc::OnGammaCorrection()
{
	// TODO: 여기에 구현 코드 추가.
	CConstantDlg dlg;
...
void CMFCApplication1Doc::OnStressTransform()
{
	// TODO: 여기에 구현 코드 추가.
	CStressTransform dlg;
...
void CMFCApplication1Doc::OnEndInSearch()
{
	int i;
	unsigned char LOW, HIGH, MAX, MIN;
...
double** CMFCApplication1Doc::OnScale(double** Target, int height, int width)
{
	// TODO: 여기에 구현 코드 추가.
	int i, j;
	double min, max;
...
double** CMFCApplication1Doc::Image2DMem(int height, int width)
{
	// TODO: 여기에 구현 코드 추가.
	double** temp;
	int i, j;
...
void CMFCApplication1Doc::OnBilinear()
{
...
void CMFCApplication1Doc::OnBlurr()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j;
...
void CMFCApplication1Doc::OnGaussianFilter()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j;
...
void CMFCApplication1Doc::OnHpfSharp()
{
	// TODO: 여기에 구현 코드 추가.
	int i, j;
...
void CMFCApplication1Doc::OnLpfSharp()
{
...
void CMFCApplication1Doc::OnDiffOperatorHor()
{
	int i, j;
...
void CMFCApplication1Doc::OnHomogenOperator()
{
	int i, j, n, m;
	...
	double CMFCApplication1Doc::DoubleABS(double X)
	{
		if (X >= 0)
			return X;
		else
	...
	void CMFCApplication1Doc::OnLaplacian()
	{
		// TODO: 여기에 구현 코드 추가.
		int i, j;
	...
	void CMFCApplication1Doc::OnNearest()
	{
		int i, j;
	...
	void CMFCApplication1Doc::OnMedianSub()
	{
	...
	void CMFCApplication1Doc::OnBubleSort(double* A, int MAX)
	{
		// TODO: 여기에 구현 코드 추가.
		int i, j;
		for (i = 0; i < MAX; i++) {
	...
	void CMFCApplication1Doc::OnSwap(double* a, double* b)
	{
		// 데이터 교환 함수
		double temp;
		temp = *a;
	...
	void CMFCApplication1Doc::OnMeanSub()
	{
